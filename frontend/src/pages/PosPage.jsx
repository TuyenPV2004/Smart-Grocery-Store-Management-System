...
const PosPage = () => {
  ...
  const addToCart = (product) => {
    ...
    setCart((prev) => {
      ...
      if (exist) {
        if (exist.cartQty >= product.quantity) {
  ...
  const updateQty = (id, delta) => {
    setCart((prev) => prev.map((item) => {
        if (item.id === id) {
            ...
            if (newQty > item.quantity) return item; // Không quá tồn kho
    ...
    <div className="flex h-screen bg-gray-100 overflow-hidden">
            ...
            {products.map(prod => (
                ...
                >
                    <div className="h-32 bg-gray-100 rounded-lg mb-3 flex items-center justify-center overflow-hidden">
                        {prod.image ? (
                            <img src={`http://localhost:8080/${prod.image}`} className="h-full w-full object-cover"/>
                        ) : <span className="text-xs text-gray-400">No Image</span>}
                    </div>
                    <h4 className="font-semibold text-gray-800 text-sm line-clamp-2 min-h-[40px]">{prod.name}</h4>
                    <div className="flex justify-between items-end mt-2">
                        <span className="text-blue-600 font-bold">{prod.price?.toLocaleString()} ₫</span>
                        <span className={`text-xs px-2 py-1 rounded ${prod.quantity > 0 ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
                            Tồn: {prod.quantity}
                        </span>
                    </div>
                </div>
      ...
      <div className="w-[400px] bg-white shadow-2xl flex flex-col h-full border-l border-gray-200">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
            {cart.length === 0 ? <div className="text-center text-gray-400 mt-10">Chưa có sản phẩm</div> : 
            cart.map(item => (
        ...
        <div className="p-5 border-t bg-gray-50">
            <div className="flex justify-between mb-2 text-gray-600"><span>Tổng tiền hàng:</span> <span>{totalAmount.toLocaleString()} ₫</span></div>
            <div className="flex justify-between mb-4 text-gray-600"><span>Giảm giá:</span> <span>0 ₫</span></div>
            <div className="flex justify-between mb-6 text-2xl font-bold text-blue-700"><span>Thanh toán:</span> <span>{totalAmount.toLocaleString()} ₫</span></div>
            
            <div className="grid grid-cols-2 gap-3 mb-4">
                <button onClick={() => setPaymentMethod("CASH")} className={`flex justify-center gap-2 py-2 rounded border ${paymentMethod==="CASH"?"bg-blue-100 border-blue-500 text-blue-700":"bg-white"}`}><Banknote className="w-4 h-4"/> Tiền mặt</button>
                <button onClick={() => setPaymentMethod("TRANSFER")} className={`flex justify-center gap-2 py-2 rounded border ${paymentMethod==="TRANSFER"?"bg-blue-100 border-blue-500 text-blue-700":"bg-white"}`}><CreditCard className="w-4 h-4"/> CK</button>
            </div>

            <button onClick={handleCheckout} disabled={loading} className="w-full bg-blue-600 hover:bg-blue-700 text-white py-3 rounded-xl font-bold text-lg shadow-lg">
