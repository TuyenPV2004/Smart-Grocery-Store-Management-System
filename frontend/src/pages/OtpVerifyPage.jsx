...
const OtpVerifyPage = () => {
    ...
    if (!email) return <div className="text-center mt-10">Lỗi: Không tìm thấy email đăng ký.</div>;

    return (
        <div className="min-h-screen flex items-center justify-center bg-gray-100">
            <div className="bg-white p-8 rounded shadow-md w-96 text-center">
                <h2 className="text-xl font-bold mb-4">Xác thực OTP</h2>
                <p className="mb-4 text-sm text-gray-600">Mã xác thực đã được gửi đến: <b>{email}</b></p>
                <form onSubmit={handleVerify}>
                    ...
                    />
                    <button type="submit" className="w-full bg-blue-600 text-white p-2 rounded">Xác nhận</button>
                </form>
            </div>
        </div>
    );
