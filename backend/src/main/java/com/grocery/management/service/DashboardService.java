...
public class DashboardService {
    ...
    public DashboardDTO getDashboardStats(Integer days) {
        ...
        List<StockSummaryDTO> stockSummary = stockService.getStockSummary();
        List<BatchExpiryDTO> batchExpiry = stockService.getBatchesWithExpiryStatus();
        ...
        List<DashboardDTO.ChartData> chartData = buildRevenueChart(startDate, safeDays);
        List<DashboardDTO.StatusCount> orderStatusChart = buildOrderStatusChart(startDate);
        List<DashboardDTO.RiskPoint> stockRiskChart = List.of(
        ...
        List<DashboardDTO.RecentOrder> recentOrders = mapRecentOrders(orderRepository.findTop5ByOrderByCreatedAtDesc());
        List<DashboardDTO.RecentOrder> pendingOrderAlerts = mapRecentOrders(
                orderRepository.findTop5ByStatusOrderByCreatedAtDesc("PENDING"));
        ...
        List<DashboardDTO.LowStockAlert> lowStockAlerts = stockSummary.stream()
                .filter(item -> "LOW_STOCK".equalsIgnoreCase(item.getStatus())
        ...
        List<DashboardDTO.ExpiringBatchAlert> expiringBatchAlerts = batchExpiry.stream()
                .filter(batch -> "EXPIRING_SOON".equalsIgnoreCase(batch.getStatus())
                        || "EXPIRED".equalsIgnoreCase(batch.getStatus()))
                .sorted(Comparator.comparingInt(batch -> batch.getDaysUntilExpiry() != null ? batch.getDaysUntilExpiry()
                        : Integer.MAX_VALUE))
                .limit(5)
        ...
        BigDecimal averageOrderValue = totalOrders > 0
                ...
                : BigDecimal.ZERO;
        DashboardDTO.MonthlyComparison monthlyRevenueComparison = buildMonthlyRevenueComparison();
